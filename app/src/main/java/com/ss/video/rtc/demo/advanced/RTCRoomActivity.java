...
public class RTCRoomActivity extends AppCompatActivity implements ConfigManger.ConfigObserver {
     ...
     */
    @NonNull
    private ByteBuffer i420ToRGBA(VideoFrame frame) {
        ByteBuffer planeDataY = frame.getPlaneData(0);
        ByteBuffer planeDataU = frame.getPlaneData(1);
        ByteBuffer planeDataV = frame.getPlaneData(2);

        int height = frame.getHeight();
        int width = frame.getWidth();
        int strideY =  frame.getPlaneStride(0);
        int strideU =  frame.getPlaneStride(1);
        int strideV =  frame.getPlaneStride(2);
        int numOfPixel = width * height;
        ByteBuffer buffer = ByteBuffer.allocateDirect(numOfPixel * 4);
        for(int i=0; i<height; i++){
            int startY = i * strideY;
            int stepU = (i / 2) * strideU;
            int stepV = (i / 2) * strideV;
            int startU = stepU;
            int startV = stepV;
            for(int j = 0; j < width; j++){
                ...
                byte byteY = planeDataY.get(Y);
                byte byteU = planeDataU.get(U);
                byte byteV = planeDataV.get(V);
                RGB tmp = yuvToRGB(byteY, byteU, byteV);
                buffer.put(index + 0, (byte) (tmp.r));
                buffer.put(index + 1, (byte) (tmp.g));
                buffer.put(index + 2, (byte) (tmp.b));
    ...
    private void onSwitchCameraClick() {
        ...
        if (mCameraID.equals(CameraId.CAMERA_ID_FRONT)) {
           mCameraID = CameraId.CAMERA_ID_BACK;
            ...
            IVideoProcessor iVideoProcessor = new IVideoProcessor() {
                @Override
                public VideoFrame processVideoFrame(VideoFrame videoFrame) {

                    ByteBuffer buffer = i420ToRGBA(videoFrame);
            ...
            mRTCVideo.registerLocalVideoProcessor(iVideoProcessor,config);
        } else {
            mCameraID = CameraId.CAMERA_ID_FRONT;
            //关闭自定义前处理
            VideoPreprocessorConfig config = new VideoPreprocessorConfig();
            config.required_pixel_format = VideoPixelFormat.kVideoPixelFormatTexture2D;
            mRTCVideo.registerLocalVideoProcessor(null,config);
